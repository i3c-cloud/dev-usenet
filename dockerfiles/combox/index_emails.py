...
def load_from_file():
    ...
    mbox = mailbox.PortableUnixMailbox(open(tornado.options.options.infile, 'rb'), email.message_from_file)
...
#curl -XPUT http://localhost:9200/.kibana/index-pattern/events-* -d '{"title" : "comm_data",  "timeFieldName": "EventTime"}'
#curl -XPUT http://localhost:9200/.kibana/config/4.1.1 -d '{"defaultIndex" : "events-*"}'    
